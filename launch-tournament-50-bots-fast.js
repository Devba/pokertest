...
/**
 * Script to create and launch a FAST tournament with 50 bots
 * - Lower starting chips (3000 instead of 10000)
 * - Hyper blind structure (3 minutes per level)
 * - Results in much faster eliminations
 * 
 * Usage: node launch-tournament-50-bots-fast.js
 */
...
async function createTournament() {
  console.log('📋 Creating FAST tournament...');
  
  try {
    const response = await axios.post(`${BASE_URL}/api/tournaments/create`, {
      name: '50 Bots Fast Championship',
      buyIn: 0,
      maxPlayers: 50,
      startingChips: 3000,          // Reduced from 10000 for faster eliminations
      blindStructure: 'hyper',      // 3 minutes per level (fastest)
      startTime: 'immediate',
      creatorWallet: 'bot-system'
    });
    
    console.log(`✅ Fast tournament created: ID ${response.data.tournament.id}`);
...
async function main() {
  console.log('🎰 50 Bots FAST Tournament Launcher');
  console.log('   - Starting Chips: 3000 (low)');
  ...
  try {
    ...
    if (status) {
      ...
      console.log(`   Starting Chips: ${status.startingChips || 3000}`);
      console.log(`   Blind Structure: ${status.blindStructure?.toUpperCase() || 'HYPER'} (3 min/level)`);
    ...
    console.log('\n==================================================');
    console.log('🎉 Fast tournament is live!');
    console.log(`   Access it at: http://localhost:3000/tournament/${tournamentId}`);
    console.log('\n⚡ With 3000 chips and hyper blinds, expect eliminations within 5-10 minutes!');
    console.log('==================================================');
